...
import { Box, Button, Container,ButtonGroup,OutlinedInput, FormControl, Grid, InputLabel, MenuItem, Select, TextField, Avatar, Paper, Typography, CircularProgress, Tab, Tabs, useTheme,  IconButton, InputAdornment } from '@mui/material';
...
import { changeEmail, changePassword, changePhoneNumber, changeRole, resetPassword, updateProfileInfo, uploadProfilePictureToStorage } from '../api/db/profile';
          ...
          <Grid item xs={12} md={7} lg={7}>
              <Paper sx={{ width: '100%' }}>
                  ...
                  <TabPanel value={value} index={1}>
                      <AccountSettings completion={completion}/>
                  </TabPanel>
              </Paper>
          </Grid>
      </Grid>
...
const AccountSettings = ({ completion }) => {
  ...
  const handleChangeRole = async () => {
    try {
      await changeRole(user,completion)
    }
    catch (error) {
      ...
      >{!user.isMerchant&&
        <Box>
          ...
          <Container sx={{ display: 'flex', justifyContent: 'end', marginTop: '20px' }}>
            <Button variant="contained" fullWidth color="primary" onClick={handleChangeRole}>Request Merchant role</Button>
          </Container>
        </Box>}
        <Box>
