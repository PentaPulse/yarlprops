  ...
  const handleOrderNow = async () => {
    try {
      if (merchantId === user.uid) {
        showAlerts(`You cant order your own ${itemType}`);
      } else {
        await addOrder(
          user,
          itemId,
          ...
          if (result.success) {
            showAlerts(
              "Your order request was sent to the merchant",
              "success"
            );
          } else {
  ...
  if (user) {
    ...
    return (
      <Box display={"flex"} justifyContent={"center"} alignItems={"center"}>
        <Typography>
          <Button onClick={setSignin}>Sign in</Button> or{" "}
          <Button onClick={setSignup}>Sign up</Button> to see seller details
        </Typography>
      </Box>
    );
  }
