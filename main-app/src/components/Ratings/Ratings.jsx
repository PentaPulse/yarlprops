...
import { db } from '../../../../admin-app/src/api/firebase';
...
export default function Ratings({ merchantId, itemId, itemType }) {
    const [item, setItem] = React.useState([])
    const [merchant, setMerchant] = React.useState(null)

    React.useEffect(() => {
        const fetchItemData = async () => {
            const q = query(collection(db, `${itemType}s`), where(`${itemType.charAt(0)}id`, '==', itemId))
            try {
                const snapshot = await getDocs(q)
                const itemlist = snapshot.docs[0].map((doc)=>doc.data())
                setItem(itemlist)
            }catch(e){
                setItem([])
            }
