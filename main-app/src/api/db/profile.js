...
export const changeRole=async(user,completion)=>{
    try {
        if (completion === true) {
...
export const changeEmail = async (user, email) => {
    ...
    try {
        ...
        await updateDoc(doc(db, 'systemusers', user.uid), { email: email.new });
...
export const changePhoneNumber = async (user, phoneNumber) => {
    try {
        ...
        await updateDoc(doc(db, "systemusers", user.uid), { phoneNumber: phoneNumber.new });
